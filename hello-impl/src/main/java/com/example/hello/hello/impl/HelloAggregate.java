...
import com.example.hello.hello.impl.HelloCommand.Hello;
import com.example.hello.hello.impl.HelloCommand.UseGreetingMessage;
import com.example.hello.hello.impl.HelloEvent.GreetingMessageChanged;
...
public class HelloAggregate extends EventSourcedBehaviorWithEnforcedReplies<HelloCommand, HelloEvent, HelloState> {
  ...
  public CommandHandlerWithReply<HelloCommand, HelloEvent, HelloState> commandHandler() {
    ...
    builder.forAnyState()
            .onCommand(HelloCommand.GetPoem.class,(state,cmd) ->
                    Effect().none()
            .thenReply(cmd.replyTo,__->new HelloCommand.Poem(state.poem)));
